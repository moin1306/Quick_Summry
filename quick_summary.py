...
Original file is located at
    https://colab.research.google.com/drive/1jG3PDnT7ftNIhXqHgVBatUrDl6G7E4UP

# QuickSummary: Simple NLP Summarizer

QuickSummary is an NLP-driven text summarization tool that efficiently condenses large articles into concise summaries using the TF-IDF algorithm. It supports multiple input methods, including manual text, .txt files, .pdf files (with page-specific summarization), and Wikipedia URLs with automated content extraction. Built with optimized Python-based text processing, it ensures scalability and high-performance analysis. The command-line interaction, paired with detailed documentation (~70 comments), enhances accessibility for users to summarize diverse text sources. The output includes clear summaries and a comparison of original versus summarized content, making it a practical solution for time-saving text analysis.
...
def pdfReader(file_path):
    ...
    choice = input("Do you want to read entire pdf ?[Y]/N  :  ").strip().upper()

    if choice == "N":
        start_page = int(input("Enter start page number (Indexing start from 0) :  "))
        end_page = int(input(f"Enter end page number (Less than {total_pages}) : "))
    else:
        start_page = 0
...
"""#Step 4
 Defining functions to create Tf-Idf Matrix
"""
...
"""#Step 5
 Using all functions to generate summary
"""
...
display(HTML(f"<pre style='white-space: pre-wrap; word-wrap: break-word;'>{summary}</pre>"))
